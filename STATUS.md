...
| id | what | next step |
|---|---|---|
| PO-67 | Front end laid out at 800x600 on a 1920x1080 display; ~77% black | **S317: the maximised front end now ACCEPTS CLICKS — `panel_click` is GREEN for the first time** (`OnSelectRlistbox row=1`), with `parity_2d` still 5/5 byte-identical. Two independent defects, both pre-existing and both invisible at 800x600: (1) **S311 gave the panel-art origin to the DRAW pass and to no click walk** — every front-end hit-test stayed at pre-S311 coordinates while the pixels moved (`rect=(810,370) vs (1100,470)`, the miss is exactly `g_ma_panel_org`); fixed by one shared `ma_ole_origin()` that reads what paint recorded. (2) **The menu rect is sized before the font exists** — `PositionRListBox`'s `Shrink`+`ResizeToFit` run while `WM_GETGLOBALFONT` still returns NULL for every index (measured), so the box was computed against the fallback 14px font at **105x100** while paint lays the same seven rows out at **292x305**; OnDraw centres rows on the rect and lets them overflow, so the outer two thirds of every label was dead. Fixed by re-running the control's own `ResizeToFit` in the draw pass, where the font is right, and keeping the width (S203 did exactly this for the height). `MA_NO_DRAWORG=1` / `MA_NO_LBREFIT=1` revert. **Next: re-flip `MA_MAXIMIZE` — but NOT on this gate alone.** S312 flipped on green gates and shipped a dead front end; `panel_click` covers the title MENU only. Verify a maximised prefs combo, a campaign-map toolbar button and a dialog scrollbar accept clicks first. |
