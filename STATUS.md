...
| id | what | next step |
|---|---|---|
| PO-67 | Front end laid out at 800x600 on a 1920x1080 display; ~77% black | **S317/S318: the maximised front end now passes the interaction suite 10/10, up from 1/10.** Three defects, all pre-existing, all invisible at 800x600 (where the panel origin is 0,0), which is why every gate stayed green while the shipped maximised build was dead to the mouse. (1) **S311 gave the panel-art origin to the DRAW pass and to no click walk** — `rect=(810,370) vs (1100,470)`, the miss is exactly `g_ma_panel_org`; fixed by one shared `ma_ole_origin()`. (2) **The menu rect is sized before the font exists** — `PositionRListBox`'s `Shrink`+`ResizeToFit` run while `WM_GETGLOBALFONT` still answers NULL for every index (measured), so the box was computed at the fallback 14px font (**105x100**) while paint lays the rows out at **292x305**; OnDraw centres rows on the rect and overflows evenly, so the outer two thirds of every label was dead. Fixed by re-running the control's own `ResizeToFit` in the draw pass, where the font is right (S203 did this for the height and left the width). (3) **`ma_ole_menu_row_point` was the LAST resolver still on raw coordinates** — and `rN` is the first step of nearly every recipe (`30,r3` = title -> Campaign), so one wrong point stranded the whole suite on the title screen and 9 gates reported `#1055 UNRESOLVED`, which reads like a broken dialog and was a click that never landed. **Evidence, both arms, same binary:** maximised 10/10 (was 1/10); unmaximised 10/10 with `parity_2d` **5/5 byte-identical**. ⚠️ **Coverage caveat:** 9 of the 10 gates run headless (`SDL_VIDEODRIVER=dummy`); only `panel_click` uses a real GL window, so the real-window maximised path is PROVEN for the title screen and INFERRED elsewhere. **Next (S319): add a maximised arm to `gates_all.sh`, THEN flip `MA_MAXIMIZE` behind it** — flipping first is precisely what S312 did, and the reason it shipped a regression was that no gate ran maximised to contradict it. `MA_NO_DRAWORG=1` / `MA_NO_LBREFIT=1` / `MA_NO_LBPARENT=1` revert the three parts. |
...
| PO-74 | Campaign buttons wrong | **S316: partly addressed by S315, NOT confirmed.** The PO's video (2026-08-27, pre-S315) shows a large WHITE RECTANGLE across the toolbar (~x 670..1030) and a grey button bearing only a faint star. A 1920x1080 capture of the current build shows **neither** — the toolbar and both filter rows render cleanly. The star is very likely one of the two files S315 excluded (`0x6607`/`0x660c`). ⚠️ **This is not a controlled comparison**: the PO's run is at campaign date 7/3/50 mid-mission-2 with briefing panels open, the gate's capture uses the pinned save at 6/25/50. Different campaign state ⇒ different buttons. **Next: reproduce the PO's state** (their save, or a frame at 6/25/50) before calling it fixed. |
