...
| id | what | next step |
|---|---|---|
| PO-67 | Front end laid out at 800x600 on a 1920x1080 display; ~77% black | **S317/S318 fixed three defects and took the maximised interaction suite from 1/10 to 10/10. S319 built the missing gate, attempted the flip, and the GATE REVERTED IT — before shipping.** Fixed and kept: (1) S311 gave the panel-art origin to the DRAW pass and to no click walk (one shared `ma_ole_origin()`); (2) the menu rect is sized before `WM_GETGLOBALFONT` answers — 105x100 against a 292x305 paint (re-run `ResizeToFit` in the draw pass, where the font is right); (3) `ma_ole_menu_row_point` was the last resolver on raw coordinates, and `rN` opens nearly every recipe, so one wrong point stranded 9 gates on the title screen. **`port/maximized_nav.sh` is the new gate arm** — a `panel_click` wrapper, because panel_click locates the menu BY ITS PIXELS in a real window; the first version drove `rN`/`#id` recipes and PASSED ITS OWN NEGATIVE CONTROL (injected clicks resolve point and hit-test in the same frame, so they agree however wrong it is). Verified both ways: PASS with the fix, FAIL with `MA_NO_DRAWORG=1`. ⚠️ **THE FLIP IS STILL OFF. Named blocker: `sysbox_exit`.** A/B, same binary — `MA_MAXIMIZE=0` finds "Yes" at (961,605) and leaves the map (99.1% changed); maximised finds it at (881,608) and does not (0.4%). The confirmation dialog reports the **same rect in both arms** (820,469,279,142) while painting "Yes" **80px** from it; the rect centre is 959, exactly where the unmaximised button is. So a **DIALOG rect does not follow the panel origin its contents are drawn at** — the PO-67 family in a path S317/S318 did not cover. **Next: fix that, then flip.** This is the second reverted flip (S312 was the first); the difference is that S312 was caught by the PO and this one by a gate. |
