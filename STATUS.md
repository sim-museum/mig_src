...
| id | what | next step |
|---|---|---|
| PO-67 | Front end laid out at 800x600 on a 1920x1080 display; ~77% black | **S317/S318 took the maximised interaction suite 1/10 → 10/10; S319 built `maximized_nav` and the gate REVERTED the flip; S320 corrected S319's diagnosis; S321 MEASURED the blocker and corrected S320's proposed fix.** ⚠️ **S321 REFUTES "the hardcoded dialog base units are the blocker".** `MA_TRACE_DLGBASE` reports the font the front end's resolution ladder actually returns (`FULLPANE.CPP:3612`): **h=26 at res 800, h=40 at res 1280 — 1.54× taller**, which IS the modal overflow in an unchanged 279x142 rect. But the *implied* base units are (20,26) and (30,40) — **2–3× the hardcoded 6x13 at BOTH layouts** — while 800x600 stays byte-identical to gold. So **6x13 is CORRECT**: Windows maps DLUs with the **TEMPLATE's** font (MS Sans Serif 8pt ≈ 6x13), which is a *different font* from the Rowan art fonts the controls draw with. Deriving base units from the art font would have rescaled every dialog and broken the one layout that works. **THE REAL FIX: `RDialog::ScaleDialog`, the engine's OWN compensator** — its comment says exactly this ("changing the dialog size at different resolutions as required in the front end"). It rescales from `scalinglookup[x][isy][0]` (design-time, statically initialised) against `[1]` (filled at runtime from `MapDialogRect`) — and **compat's `CWnd::MapDialogRect` is `{}` (`afxwin.h:946`)**, so the runtime column is never populated and the compensator is inert. Same family as BoB S37/S315: a stubbed compat function silently disabling a real engine mechanism. **Next: implement `MapDialogRect` and let `ScaleDialog` run**, with `parity_2d` byte-identity at 800x600 as the gate (that layout must not move). Smaller and more faithful than rescaling template rects. ⚠️ Also fixed in S321: the first version of this probe measured the screen DC's CURRENT font and reported an identical 12x8 at both layouts — a plausible WRONG number. `ma_gdi_font_metrics(hfont,…)` now measures the font the ladder RETURNS, and the ladder is refactored so probe and return value cannot diverge. Verified 4/4 clean (parity 5/5 byte-identical, panel_click, maximized_nav, sysbox_exit). |
