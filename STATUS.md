...
| id | what | next step |
|---|---|---|
| PO-67 | Front end laid out at 800x600 on a 1920x1080 display; ~77% black | **S317/S318: maximised suite 1/10 → 10/10. S319: gate built, flip reverted BY the gate. S320→S321→S322 each corrected the previous fix. S324 MEASURES the handler and CONFIRMS S322.** `MA_TRACE_GLOBALFONT=1` (new) names the runtime class answering `WM_GETGLOBALFONT` and the height of the font it returns, at the single dispatch site (`RDIALOG.CPP:2246`). With the quit modal up, maximised: <br>`[globalfont] 14RFullPanelDial fontnum=10 -> font h=40 w=30` <br>`[globalfont] 10CSystemBox    fontnum=0  -> font h=14 w=11` <br>`[globalfont] 5CLoad          fontnum=2  -> font h=14 w=11` <br>⭐ **`RMdlDlg` NEVER APPEARS.** Its `OnGetGlobalFont` returns set `[0]` unconditionally and is virtual (`RDIALOG.H:493`), so the modal's text SHOULD be ~h=20 — instead `RFullPanelDial`'s resolution ladder answers with **h=40**, which is exactly the 1.54× overflow that overlaps YES/CANCEL. S322 confirmed. ⚠️ **Second defect, new:** `RFullPanelDial fontnum=10 -> font h=0 w=0` — a **NULL font slot**; `g_AllFonts[10][…]` is unpopulated and callers get a zero-metric font. **Next: find WHICH draw path paints the modal's text.** `ma_static_draw` sets `c->m_maParent = parentWnd` and the control asks `GetParent()`, so anything drawn by the modal's own `ma_ole_draw_toolbar(this,…)` would ask `RMdlDlg` — it doesn't, so the modal's controls are hosted under the PANEL and painted by `ma_ole_draw_all`. Fix = make the modal's controls resolve through the modal (the S317 "attach the parent before the call" pattern), NOT a geometry or base-unit change — both already refuted (S321 by measurement, S322 by Wine ground truth: dialogs stay small at 1080). `parity_2d` byte-identity at 800x600 remains the gate. |
