...
| id | what | next step |
|---|---|---|
| PO-67 | Front end laid out at 800x600 on a 1920x1080 display; ~77% black | **S317/S318 took the maximised interaction suite 1/10 -> 10/10. S319 built the gate arm and the gate REVERTED the flip. S320 CORRECTS S319's diagnosis and names the real, systemic cause.** ⚠️ **S319 WAS WRONG.** It claimed "a DIALOG's rect does not follow the panel origin". It does. Measured: the quit confirmation's buttons are at **identical rects in both arms** — Save (830,587,78,24), Yes (922,587,78,24) — and the dialog reports (820,469,279,142) either way. The tell I skipped was in my own log: `[modal] click -> taken` said the click WAS taken, and I never asked *by what*. It was taken by **Save** (id 1051), not Yes (id 2125). **The real defect (evidence: `port/ref/po67/modal_*.png`): control rects are RESOLUTION-INDEPENDENT while fonts are RESOLUTION-DEPENDENT.** `ma_ole_dialog_extent` sizes the modal from its hosted controls' template rects (DLU->px, fixed 279x142 at any resolution), while the captions are drawn with the global font, which scales with resolution (S317 measured the same font as 14px at 800x600 and 43px at 1080). At 800x600 they agree; maximised the text grows ~40% and the rects do not, so "QUIT GAME" is clipped, "Are you sure?" wraps to two lines, and **YES and CANCEL overlap into "YE**S**CANCE"**. A user would misclick this. **This is the S317 listbox defect generalised** — there the rect was computed with the small font while paint used the big one; the cure was to make the rect follow the FONT (re-run `ResizeToFit` where the font is right). That cure now has to be applied to dialogs and buttons generally, not just the menu listbox. **That is the real precondition for the flip**, and it is epic-sized, not a one-line fix. ⚠️ Also note `port/sysbox_exit.sh`'s locator is fragile independently: it takes the middle of three yellow runs on the single yellowest row, and maximised that row yields FOUR runs including a 2px fragment, which is the pixel it clicked. Harden it (band-aggregate the button row, drop sub-15px runs) so it fails for the stated reason. |
