...
| PO-82 | 🔴 **Objects lose their textures ~30 s into a dogfight — everything renders WHITE** | **PO 2026-08-29**, after setting **primary graphics** and **bilinear/trilinear filtering to ALL** in preferences. Screenshots `Screenshot From 2026-08-29 10-52-01/10-52-31/10-52-45/10-54-29/10-55-17.png`: the cockpit, canopy frame and airframe render pure white; the ADI and gunsight (separate art) still texture correctly, so it is not a global GL failure. <br>⭐ **THE CODE ALREADY NAMES THIS SYMPTOM.** `bob_video.cpp:1719-1723` (S153): *"If the driver has not got it, stay on `GL_LINEAR` rather than bind a texture with an incomplete mip chain — **that renders WHITE**. `MA_NO_MIPMAP=1` reverts."* So "white objects" is the known signature of **a mipmap min-filter on a texture whose chain is incomplete**. <br>**FIRST THING TO TRY, and it is one env var:** run with `MA_NO_MIPMAP=1`. If the white goes away, the cause is confirmed and the fix is in the chain, not the filter. <br>**Where to look, in order:** <br>1. `ma_gl_mip_and_filter()` is called **only at upload time** (`:1786`, `:2100`). Every `glTexImage2D` site either sets plain `GL_LINEAR` (`:1159/1208/1298/1361`) or calls the helper (`:1780-1786`, `:2060-2100`), so no site obviously skips it — **but `glTexImage2D` replaces LEVEL 0 ONLY**. A texture re-uploaded at a DIFFERENT SIZE keeps stale higher levels and becomes incomplete → white. A dogfight is exactly when textures get re-uploaded (damage/skin updates). **Instrument the re-upload path for a size change against an existing mip chain.** <br>2. ⚠️ **The game's own filter state is NOT implemented in this port:** `D3DTSS_MINFILTER` / `D3DTSS_MAGFILTER` / `D3DTFN_*` / `D3DTFP_*` have **zero hits** in `bob_video.cpp`. So the PO's preference change may not reach GL at all — in which case the trigger is something else the preference ALSO changes (texture detail/size), which loops back to (1). Do not assume the setting is what is being applied. <br>3. Sister-port precedent: **BoB R3.5** fixed a `HINT_TRILINEAR` crash caused by *a missing attached mip-level chain* — same family, and its fix built the sub-surface chain rather than disabling filtering. |
