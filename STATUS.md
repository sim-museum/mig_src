...
| PO-83 | **Variants screen before a 1-on-1 mission is empty** | **PO 2026-08-29**, screenshot. Choosing **Variants** before the 1v1 mission shows the pilot/aircraft photo art with only the labels **"F86 1"** and **"F86 2"** and a **Back** button — no variant content. <br>⚠️ **Same shape as BoB R3.8** (*"pre-3D campaign screen shows NO aircraft list"*), where the answer was measured rather than guessed: the list was **created, hosted, visible, drawn at a real rect, and populated with correct data** — and the defect was in PAINTING THE ROWS. **Do not assume this list is unpopulated.** <br>**Discriminating measurement first:** dump what the variants dialog hosts — control ids, `visible`, and DLU rects — and whether the rows carry data. In MA that is `MA_TRACE_OLE` / the `#ID` control-point resolvers; the two outcomes need different fixes: <br>• rows present with data → a paint/row-render defect (BoB R3.8's answer); <br>• no rows → a population defect, look at what fills the variant list for a QUICK-MISSION 1v1 (not the campaign path). <br>⚠️ Note the two labels DO render ("F86 1", "F86 2"), so text drawing on that screen works — which already argues against "nothing is drawn" and toward the list itself. |
