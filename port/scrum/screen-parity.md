...
| 10 | 02-21 | In-flight cockpit (F-86) | `flight_cockpit.png` (FLY + `MA_DUMP_BACK=100`) | PARTIAL | Scene/terrain/sky/gunsight-glass render; deviations: cockpit frame + instrument panel render flat black (POV cockpit art untextured); padlock-ADI inset black rectangle (top-right); scene state differs (above cloud deck vs gold over terrain) — needs same-view recapture. **S72 narrowing:** the cockpit renders as a CRISP black SILHOUETTE (geometry rasterizes; only the FILL is black), and the software rasterizer HAS the image-span fillers (`XASM_ImageHoriLine*` etc. in `ma_xasm.nasm`) while world terrain + the gunsight texture render — so it is NOT a missing primitive and NOT `textureQuality` gating. Root cause narrowed to the **cockpit-specific imagemaps resolving to black (index 0) — not loaded/bound**, on the `btree::drw_cockpit` (`COCKPIT_OBJECT`) shape path. Scoped fix target for S73: trace the cockpit shape's per-poly `Image_Map.GetImageMapPtr` binding vs a rendering world poly. |
