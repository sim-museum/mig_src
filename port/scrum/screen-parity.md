...
| 7 | 01-23 | Preferences → Controls | `prefs_controls.png` | PARTIAL | As #2, plus: `Calibrate` button missing; Enable/Use-for-FF checkbox glyphs missing; "Dead Zone:"/"Airframe"/"Input Devices:" labels missing; joystick axis names empty ("active joystick : &" vs "Axis 0 & Axis 1") — DI axis-name enumeration not filled |
| 8 | 01-32 | Preferences → Others | `prefs_others.png` | PARTIAL | As #2, plus ~6 row labels missing (Control SFX / Ambient SFX / G Effects / Injury Effects / White Outs / Auto Vectoring) — combos render, labels don't |
| 9 | 01-45 | Quick Mission setup | `quickmission.png` | PARTIAL | Rows/combos/mission text/Back-Variants-Fly all render; deviations: mission text not word-wrapped to the panel (runs off right edge); stray combo artifact ~(590,165); Scenario/UN radio row missing; "I.D." label vs gold none (resource delta?) |
...
3. **Missing static labels on some panels** (#7, #8) — likely the (dlgId, ctrlId)-scoped
   rect/label lookup keystone from BoB S123 (cross-port note 13 §2) — repeated static ids
   take rects from other dialogs. Check `ma_olecontrol`/`ma_dlgtmpl` scoping.
