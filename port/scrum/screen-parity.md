...
| 15 | (I4 gold, 2026-07-19) | Player Log OOB dialog over map | `map_playerlog.png` — CAMP + `MA_OOB_PLAYERLOG=1 MA_SHOT=160` (S60: the S58 GL-free `MA_SHOT` path replaces `BOB_DUMP_FRAME=280`, which no longer reached the dump within a sane timeout) | PARTIAL (S60: substantial progress, acceptance NOT met) | **S60 fixed, in-capture:** the Career **"Name" label + Name edit box** now render (template-declared controls are instantiated at last), and a **title-bar element** appears. **S60 root-caused + mechanism landed but NOT yet visible:** the RTabs OCX is hosted (`ma_oletabs.cpp`, CLSID `4a1e1986`) and all three tabs register with the gold captions (`AddTab "Career"/"Log of Missions"/"Last Mission"`) with the real 297x31 tab art loaded from **RTabs.ocx's own PE** — but the tab bar is not composited at the right screen offset, so it does not appear. Two engine gaps were behind three of the four original deviations: (a) **template-declared OCX controls that no dialog class DDX_Control-binds were never created** (IDJ_TITLE=RButton in IDD 276, IDJ_TABCTRL=RTabs in IDD 130) — the S57 static-only hoster is now kind-driven; (b) **no RDialog in a tree ever learned its own size** (ctor zeroes homesize/viewsize, the refresh line is commented out at RDIALOG.CPP:147) so RDialog::OnSize gave the tab control a zero-width MoveWindow — fixed via `MaSeedTemplateSize()` (tab host 0x0 -> 420x258). Remaining (named): tab bar + title bar **compositing offset** (suspect `OnGetXYOffset`, whose parent-walk only accumulates when `parent->artnum == artnum` and every node here is artnum 0); `?`/`✓` title buttons not yet identified in the template; Sorties/Combats/Kills/Losses table still absent (never pulled — the other half of I4) |
