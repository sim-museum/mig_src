#!/usr/bin/env python3
# Port common single-variable x87 FPU _asm blocks to libm/__builtin (BOB_LINUX is
# defined, so these run on Linux). Conservative: only well-known fixed shapes.
import sys, re
W = r"[ \t]*"          # horizontal ws
C = r"(?:[ \t]*//[^\n]*\n)*"   # optional comment lines
def asm(body): return r"_asm\s*\{?\s*" + body + r"\s*\}"
SUBS = [
  # fld V; fsqrt; fld1; fpatan; fstp V  -> atan(sqrt(V))
...
for f in sys.argv[1:]:
    ...
    if n: open(f,"w",encoding="latin-1").write(s); print(f"{f}: ported {n} FPU asm blocks"); total+=n
print("total:",total)
