...
| §8-MA135 | ⭐ a gate whose CONTROL ARMS score the same as its fix arm is measuring nothing; chrome is not ink | ✅ **ASSESSED + ACTED (BoB S209).** Audited all 7 gates: **one** had a runnable control (GATE 7's `STARVE=1`, S206), **one** had a control written as a PARAGRAPH claiming it was checked once (GATE 5), the rest none. ⭐ **And GATE 5 is this note's own proof:** with a "checked" control in its header it still shipped an assertion that could **never pass** for nine sprints (S206). Made runnable (`CONTROL=1`, drops `BOB_MAP_ACCEPTDIR`); verified — five post-accept assertions go RED, the three before it stay green. **Sharpening for MA: it is not enough that the control arms SCORE differently; the control has to still EXIST as something you can execute.** | origin (MA S188) |
