...
| PO-21 | As a player, front-end panels **replace** the previous screen instead of layering on it. | 5 | Opening a campaign panel shows only that panel; clicking around does not accumulate text. | ✅ **CLOSED S128 (2026-08-15)** — a B6 regression, PO-reported with screenshots. Panel art is a fixed 800×600 bitmap drawn at (0,0); before B6 the canvas WAS the art size so each panel wiped the last. With the canvas at display resolution the art covered one quadrant of 1920×1080 and the other three kept the previous panel's pixels. Art is now CENTRED with the margins cleared (gold's model — its briefing panel sits centred with dark margins) and hosted controls take the same origin. |
| PO-22 | As a player, the campaign map has its **ruler/scale**, so I can judge distances. | 3 | The map scale bar is drawn and updates with zoom. | 🔨 **PO-requested 2026-08-15.** `CScaleBar` / the Scale Toolbar is one of the two toolbar clusters S111 identified as present-but-unhosted (0 hosted controls, draws itself, nothing calls it). Also listed in the game's own help text as one of the five dockable toolbars. |
...
**Sprint Review (PO pre-approved ceremony, logged 2026-08-15):** `port/scrum/sprint-128.md`.

- **A B6 regression the PO caught within minutes of the win.** Front-end panel art is a fixed
  800×600 bitmap drawn at (0,0). Before B6 the canvas WAS the art size, so every panel wiped the
  one before it. With the canvas at the display resolution the art covers one quadrant of
  1920×1080 and **the other three keep the previous panel's pixels** — so clicking around only ever
  added more text, and the Fly button was buried.
- Measured, not guessed: `[panelart] art 800x600 at (0,0) on canvas 1920x1080`.
- **Fixed the way gold does it:** the art is centred with the margins cleared (gold's briefing panel
  sits centred with dark margins), and hosted controls take the same origin so they land on the art
  instead of in the corner.
- **Gates:** parity 5/5 byte-identical (the panel origin is 0 at the default resolution, where the
  canvas still equals the art size) · sweep 9 OPEN/0 CRASH · map drag PASS · map click · sysbox ·
  help click.
