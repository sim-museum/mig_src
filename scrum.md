...
| PO-56 | As a player, the Ins Wave dialog is fully on screen, lets me set the time, and can be closed. | 8 | The wave dialog appears wholly within the window, its Time Over Target field accepts a time, and it has a reachable close/OK control. | 🔨 **NEW — PO, Wonju playthrough.** *"No way to set time in Ins Wave dialog, no way to exit this dialog."* The PO's capture shows it **running off the left edge of the window**: the title reads `…PLY DUMP`, the tab row `…A Cover / Air Cover`, and the `Time Over Target:` label is cut. `Ins Wave` / `Del Wave` are visible but the title bar (with its tick/close bands) is off-screen, so the dialog cannot be dismissed. Same class as **K5**'s MISSION FOLDER, which gold also draws past the left edge — but there it was cosmetic; here it makes the dialog unusable. Likely one cause with PO-55. |
...
| PO-58 | As a player, tapping the wheel brakes releases them and the aircraft rolls. | 8 | With 100% throttle, tapping `,` and `.` releases the parking brakes and the aircraft accelerates. | 🔨 **OPEN — but the brakes are PROVEN INNOCENT (S180).** Traced end to end from the PO's own keypresses:\n`[brake] SDL DOWN scancode=54 -> dik=0x33 diKbAcquired=1` → `[brake] left=1 right=0` → `SDL UP` → `left=0 right=0`. The real keyboard path works perfectly: SDL event → DIK → `KeyHeld3d` → `pModel->LeftWheelBrake`. **So the wheel brakes are not what is holding the aircraft**, and the earlier framing ("tapping does not release them") was wrong. **Next measurement, not yet taken:** `thrustpercent` during the PO's run — no throttle trace was enabled. ⚠️ **Hypothesis, explicitly unverified:** with a joystick attached, `Analogue::PollPosition` writes `axisvalues[AU_THROTTLE]` from the slider on EVERY poll, which would override any keyboard throttle; if the PO's slider sits at idle the engine is at idle whatever the keyboard did. One check settles it: slider to maximum before takeoff. |
...
| PO-60 | As a player, the keyboard works as soon as a flight starts, without alt-tabbing. | 8 | Keys reach the sim from the moment the 3D view appears. | 🔨 **NEW (S181) — PO, and it probably explains most of PO-58.** *"\",.\" didn't work but neither did F2. I had to ALT TAB away and back for F2 to work."* **F2 is a view key with nothing to do with brakes**, so it was not the brakes failing — **no keyboard input was reaching the sim at all** until the window regained focus. That makes the earlier "tapping brakes does nothing" report a symptom of this, not of the brake path (which S180 proved correct end to end). Suspect: SDL keyboard focus / `SDL_WINDOWEVENT_FOCUS_GAINED` handling, or the DirectInput device being acquired before the window has focus. ⚠️ Note the trace that *did* capture working brake presses was almost certainly taken **after** the PO alt-tabbed — so a clean repro must capture the state from the first frame of the flight. |
| PO-61 | As a player, I can load and play a `.cam` replay. | 8 | Selecting a replay from the Replay screen plays it back. | 🔨 **NEW (S183) — PO: selecting a replay CRASHES.** Backtrace symbolised from the PO's session (`/tmp/ma_session_po.log`), SIGSEGV `fault_addr=0x36b8b1de`:\n`Rtestsh1::Launch3d` → `Inst3d::Inst3d` → `Replay::LoadFinalPlaybackData` → `LoadBlockHeader` → `LoadItemAnims` → `shape::ResetAnimData_NewShape` → `shapestuff::GetShapePtr(unsigned short)` → `fileblock::getdata()`. So a **ShapeNum read out of the replay file resolves to a bad fileblock** and `getdata()` dereferences it. The eight `Ian*.cam` files ship with the game and are already in the Linux run dir (no copy needed — the port runs out of the Wine `drive_c`). **The whole `_Replay` subsystem is exercised by NO gate.** Also reported: something is superimposed on the Replay screen (probably the same class as PO-59, on a transition `DestroyPanel` does not cover). |
| PO-62 | As a player, the campaign map dialogs are not cut off at the left edge of the screen. | 5 | Dialog titles and fields are fully visible. | 🔨 **NEW (S183).** The PO's capture shows `PROFILE OVERVIEW`, `…PLY DUMP`, `…er Target:` and `MISSION FOLDER`'s `…ply Dump` all cut at the left. ⚠️ **NOT the S182 dialog clamp** — that fired in this very session but only for `x=-3`/`y=-3`, far too small to explain it. Next measurement: the **SDL window position and size vs the canvas size** — the screenshot includes the desktop panel, so the game window may simply be placed partly off the left of the display, which is a window-geometry issue rather than a dialog one. |
| PO-63 | As the team, a menu row past the control's height is still reachable by recipe. | 3 | `#ID:rN` / `,rN` resolves a clickable point for every drawn row. | 🔨 **NEW (S183), found while trying to reach the Replay screen.** The title menu traces `[clickrow] row=4 -> (582,335) [listbox (530,210) 105x100, 7 rows, listH=199 rowH=28]` — **7 rows × 28px = 196px drawn inside a control only 100px tall**, so rows past the third are painted OUTSIDE the control's own rect (210–310) and the injected click at y=335 hits nothing. The PO reaches Replay with a real mouse; **no recipe can**, which is why the Replay path has no gate. Same family as **PO-43** (a list overflowing its dialog), now on the title menu. Blocks automated testing of PO-61. |
