...
| PO-13 | As a player, pressing a number key **inside** an in-flight menu selects that option, so the menus are usable and their sub-screens reachable. | 5 | Driving `R` then `3`, or `M` then `2`, reaches the submenu (gold `full` @ ~190 s shows the combat submenu; the map's option 2 is `waypointMapScr`, whose `UpdateWaypointDisplay` draws the gold's Rendezvous/Ingress/Initial-Point table). | ⬜ **S104/S105 open item, seen twice.** The taps ARE delivered (`[key] DOWN scancode=0x04 -> action index=108` = RPM_30) and the throttle consumer in `KEYFLY.CPP` is correctly guarded by `if (!OverLay.pCurScr)`, so it is not eating them; the option poll in `RenderInfoPanel` never reports a selection. Complication: the in-flight **pump** rate is low enough that taps 30 pumps apart land *seconds* apart — several attempts simply arrived after the (5 s) menu had closed. Drive the option key from the screen's own frame loop rather than the pump counter |
