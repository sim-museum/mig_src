...
| PO-57 | As a player, my own name appears in the frag pilot roster. | 5 | The player occupies a pilot slot on the frag screen and is named there. | 🔨 **NEW — PO, Wonju playthrough.** *"my name missing on fly roster screen."* The capture shows the **first pilot field blank** while every other slot is filled (Ray J. Dickinson, John H. Heyman, …). S173 gated the roster by COUNTING names (≥8 distinct) and by asserting the seat write (`MMC.playeracnum`) — **neither would notice an empty first slot**, which is exactly the gap the PO found. The player's name comes from the pilot record, not the roster list. |
| PO-58 | As a player, tapping the wheel brakes releases them and the aircraft rolls. | 8 | With 100% throttle, tapping `,` and `.` releases the parking brakes and the aircraft accelerates. | 🔨 **NEW — PO, Wonju playthrough.** *"tapping brakes does not cause aircraft to move — stuck on airstrip with 100% throttle as other aircraft take off."* ⚠️ **My synthetic driver DOES release them** (`BOB_AUTOFLY=takeoff` reaches 143 kt), so the harness path works and the **real keyboard path does not** — the difference is `kb_push()` vs a real SDL key event. That difference is the bug, and it also means **every gate that drives keys is testing a path the player does not use** (§8-MA124 again, from the other side). Correct behaviour per the PO: no movement until BOTH brakes are tapped. |
