...
| PO-57 | As a player, my own name appears in the frag pilot roster. | 5 | The player occupies a pilot slot on the frag screen and is named there. | 🔨 **ROOT CAUSE LOCATED (S180), fix needs a decision.** Traced from the PO's own session: **`MMC.PlayerName="" (len 0)`**. Not a rendering bug — on the pinned save it reads `"Name"` and `MA_TRACE_TEXT` shows it drawn (white, black drop-shadow, 42 white px survive into the frame). **`MMC.PlayerName` has exactly ONE writer in the whole tree: `CLockerRoom::…` (LOCKER.CPP:398), and that screen is reached only from `LockerRoomInit`, which hangs off the MULTIPLAYER ready-room chain** (its menu is `IDS_QUICKMISSION1→LobbyCheck` / `IDS_CONTINUE→CreatePlayer`, next to `visitorsbook`). So nothing sets it in a single-player campaign. ⚠️ **Open question for the PO, not a guess:** does the Windows game ask for a pilot name when you start a single-player campaign, and on which screen? Inventing a name-entry flow would be making up behaviour (§8-MA132). |
...
| PO-59 | As a player, leaving a campaign returns me to a clean title screen. | 5 | Exiting the campaign shows the title screen with nothing drawn over it. | 🔨 **NEW — PO, Wonju playthrough.** *"after exiting campaign pilot roster is superimposed on landing page."* The capture shows the frag roster rows drawn over the MiG Alley title art and menu. **This is PO-51 recurring on a different transition** — S169 scoped the campaign map's dialogs off the frag screen with `ma_ole_set_parent_scoped`; the frag screen's own controls are evidently not scoped off the TITLE screen. S169 fixed one direction of the same fault. |
