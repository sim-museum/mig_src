...
| PO-52 | As a player, my aircraft accelerates down the runway and takes off. | 8 | From a runway start at 100% thrust the aircraft accelerates past rotation speed (~100 kt per the PO's script) and leaves the ground. | 🔨 **NEW (S174), isolated with a reproducible A/B.** From the campaign runway start the aircraft accelerates **0 → 20 kt and then plateaus, indefinitely**, at full military power. Measured, so the usual suspects are already excluded: the throttle command **lands** (`RPM_00 → thrustpercent=100`, 188 times in one run); the player **has manual control** (`controlmode=MANUAL`, `movecode=AUTO_FOLLOWWP` — *not* on the AI takeoff rail, which was the first hypothesis and was wrong); the wheel brakes are `KeyHeld3d`, i.e. **off unless held**, so they are not the constraint; and the **flight model itself is fine** — the Hot Shot airborne start flies at **503 kt / Mach 0.84 / 15,966 ft** with speed and altitude evolving normally under the same build. The difference is the **ground roll**: on the ground, thrust is not producing acceleration past 20 kt. Next: undercarriage friction / ground-reaction, and whether weight-on-wheels ever clears. Drive: `BOB_AUTOFLY=takeoff` + `MA_TRACE_HUD=<n>`. |
