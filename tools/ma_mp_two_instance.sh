...
( cd "$GD" && timeout -s INT "$SECS" env MA_DUMP_MENU=1 BOB_CLICKSEQ_MS=1 MA_TRACE_CLICK=1 MA_TRACE_3D=1 MA_TRACE_ACCOUNT=1 MA_TRACE_ADDPLAYER=1 \
...
hpid=$!
sleep "$CLIENT_DELAY"
( cd "$GD" && timeout -s INT "$((SECS - CLIENT_DELAY))" env MA_DUMP_MENU=1 BOB_CLICKSEQ_MS=1 MA_TRACE_CLICK=1 MA_TRACE_3D=1 MA_TRACE_ACCOUNT=1 MA_TRACE_ADDPLAYER=1 \
