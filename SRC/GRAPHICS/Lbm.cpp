...
//DEADCODE JIM 29/03/99 			return (Unpack3Row(c,width,x<<2,y,minx<<2,maxx<<2));
	...
	// Cross-port (BoB S47): the shared lbmcpp.h unpack now bounds control-byte reads
	// against `cend`. This generic row-unpacker takes only a start pointer (no source
	// size) and is currently uncalled (declared in DISPLAY.H, no callers), so there is
	// no real end to bound against -- set a max-address sentinel to keep LBM_INBOUNDS
	// inert here while satisfying the shared macro. The bounded path is FixLbmImageMap.
	const UByte* const cend = (const UByte*)-1;
#endif
