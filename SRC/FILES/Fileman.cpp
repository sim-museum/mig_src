...
//DeadCode DAW 25Feb97 	INT3();
	...
	//delete fb;
		...
		if (errhandle)
		{
//DEADCODE DAW 28/03/99 		#ifdef	___MSVC__
//DEADCODE DAW 28/03/99 			fprintf(errhandle,"%d",Inst3d->timeofday);
...
{
	SLong	retval;
    __asm__ __volatile__("call 1f\n1:	pop %0" : "=a"(retval));  // Linux/GCC port: get-EIP
    return retval;
}
...
{
	SLong*	retval;
    __asm__ __volatile__("mov %%esp, %0" : "=r"(retval));  // Linux/GCC port: get-ESP
    return retval;
}
...
void fileman::InitFileSystem()
{
	...
	delete loaddirlist(FIL_640resDIR);   // Res640.dir is the only res registry shipped here
