...
extern "C" void ma_ole_create(void* client, const void* clsidPtr, void* parent) {
    ...
    if (clsid_is(clsid, 0x48814009 /*RListBox*/)) {
        ...
        /* S329-S2 (PO-83): the variants dialog's two CRRadio groups ARE populated (S329-S1 measured
           three buttons reaching each control) yet nothing is drawn, and CRRadioCtrl::OnDraw never
           fires even under real GL. The drawer is only reached for controls classified HERE as
           CT_RADIO, so record the classification and let the dispatcher record the call. If the
           variants controls never appear in this line, they are not hosted as radios at all and the
...
void ma_ole_draw_all(void* screenHdc) {
    ...
    for (std::map<void*, Hosted>::iterator it = m.begin(); it != m.end(); ++it) {
        ...
        if (h.type == CT_STATIC) {
            ...
            /* S329-S2 (PO-83: "when I clicked varients before 1 on 1 mission I got empty screen").
               THE DEFECT: this dispatcher had no CT_RADIO branch, so a hosted, classified,
               populated radio group was never drawn by the MAIN drawer. Only
               ma_ole_draw_toolbar had the branch, which is why radios appear on toolbar dialogs
               and nowhere else.
               Measured before fixing: the variants dialog hosts 4 controls as CT_RADIO and
               CRRadioCtrl::AddButton receives 3 correct strings for each group ("F86A"/"F86E"/
